...
func Orgs_create_issue_typeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.GeneratedType_Organization_create_issue_type
		...
		url := fmt.Sprintf("%s/orgs/%s/issue-types", cfg.BaseURL, org)
...
func CreateOrgs_create_issue_typeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_orgs_org_issue-types",
		mcp.WithDescription("Create issue type for an organization"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Orgs_create_issue_typeHandler(cfg),
	}
}
