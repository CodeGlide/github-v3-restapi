...
func Users_list_attestations_bulkHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/users/%s/attestations/bulk-list%s", cfg.BaseURL, username, queryString)
...
func CreateUsers_list_attestations_bulkTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_users_username_attestations_bulk-list",
		mcp.WithDescription("List attestations by bulk subject digests"),
		...
		mcp.WithString("predicate_type", mcp.Description("Input parameter: Optional filter for fetching attestations with a given predicate type.\nThis option accepts `provenance`, `sbom`, or freeform text for custom predicate types.")),
		mcp.WithArray("subject_digests", mcp.Required(), mcp.Description("Input parameter: List of subject digests to fetch attestations for.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Users_list_attestations_bulkHandler(cfg),
	}
}
