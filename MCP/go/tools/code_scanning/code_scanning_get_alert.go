...
func Code_scanning_get_alertHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/code-scanning/alerts/%s", cfg.BaseURL, owner, repo, alert_number)
		...
		var result models.GeneratedType_Code_scanning_alert
...
func CreateCode_scanning_get_alertTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_code-scanning_alerts_alert_number",
		mcp.WithDescription("Get a code scanning alert"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Code_scanning_get_alertHandler(cfg),
	}
}
