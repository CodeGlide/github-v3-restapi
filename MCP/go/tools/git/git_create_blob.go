...
func Git_create_blobHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/git/blobs%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateGit_create_blobTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_git_blobs",
		mcp.WithDescription("Create a blob"),
		...
		mcp.WithString("content", mcp.Required(), mcp.Description("Input parameter: The new blob's content.")),
		mcp.WithString("encoding", mcp.Description("Input parameter: The encoding used for `content`. Currently, `\"utf-8\"` and `\"base64\"` are supported.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Git_create_blobHandler(cfg),
	}
}
