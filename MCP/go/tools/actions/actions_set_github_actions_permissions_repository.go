...
func Actions_set_github_actions_permissions_repositoryHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/permissions%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateActions_set_github_actions_permissions_repositoryTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_repos_owner_repo_actions_permissions",
		mcp.WithDescription("Set GitHub Actions permissions for a repository"),
		...
		mcp.WithBoolean("enabled", mcp.Required(), mcp.Description("Input parameter: Whether GitHub Actions is enabled on the repository.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Actions_set_github_actions_permissions_repositoryHandler(cfg),
	}
}
