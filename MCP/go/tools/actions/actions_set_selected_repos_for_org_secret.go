...
func Actions_set_selected_repos_for_org_secretHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/actions/secrets/%s/repositories", cfg.BaseURL, org, secret_name)
...
func CreateActions_set_selected_repos_for_org_secretTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_orgs_org_actions_secrets_secret_name_repositories",
		mcp.WithDescription("Set selected repositories for an organization secret"),
