...
func Actions_generate_runner_jitconfig_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/runners/generate-jitconfig%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateActions_generate_runner_jitconfig_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_actions_runners_generate-jitconfig",
		mcp.WithDescription("Create configuration for a just-in-time runner for a repository"),
		...
		mcp.WithArray("labels", mcp.Required(), mcp.Description("Input parameter: The names of the custom labels to add to the runner. **Minimum items**: 1. **Maximum items**: 100.")),
		mcp.WithString("name", mcp.Required(), mcp.Description("Input parameter: The name of the new runner.")),
		mcp.WithNumber("runner_group_id", mcp.Required(), mcp.Description("Input parameter: The ID of the runner group to register the runner to.")),
		mcp.WithString("work_folder", mcp.Description("Input parameter: The working directory to be used for job execution, relative to the runner install directory.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Actions_generate_runner_jitconfig_for_repoHandler(cfg),
	}
}
