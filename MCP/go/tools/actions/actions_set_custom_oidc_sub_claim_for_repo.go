...
func Actions_set_custom_oidc_sub_claim_for_repoHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/actions/oidc/customization/sub%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateActions_set_custom_oidc_sub_claim_for_repoTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_repos_owner_repo_actions_oidc_customization_sub",
		mcp.WithDescription("Set the customization template for an OIDC subject claim for a repository"),
		...
		mcp.WithArray("include_claim_keys", mcp.Description("Input parameter: Array of unique strings. Each claim key can only contain alphanumeric characters and underscores.")),
		mcp.WithBoolean("use_default", mcp.Required(), mcp.Description("Input parameter: Whether to use the default template or not. If `true`, the `include_claim_keys` field is ignored.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Actions_set_custom_oidc_sub_claim_for_repoHandler(cfg),
	}
}
