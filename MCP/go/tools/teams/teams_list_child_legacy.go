...
func Teams_list_child_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/teams/%s/teams%s", cfg.BaseURL, team_id, queryString)
...
func CreateTeams_list_child_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_teams_team_id",
		mcp.WithDescription("List child teams (Legacy)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_list_child_legacyHandler(cfg),
	}
}
