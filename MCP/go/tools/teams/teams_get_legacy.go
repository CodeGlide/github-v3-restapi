...
func Teams_get_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var result models.GeneratedType_Team_full
...
func CreateTeams_get_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_teams_team_id_2",
		mcp.WithDescription("Get a team (Legacy)"),
		mcp.WithNumber("team_id", mcp.Required(), mcp.Description("The unique identifier of the team.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Teams_get_legacyHandler(cfg),
	}
}
