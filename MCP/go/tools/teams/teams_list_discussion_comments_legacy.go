...
func Teams_list_discussion_comments_legacyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/teams/%s/discussions/%s/comments%s", cfg.BaseURL, team_id, discussion_number, queryString)
		...
		var result []models.GeneratedType_Team_discussion_comment
...
func CreateTeams_list_discussion_comments_legacyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_teams_team_id_discussions_discussion_number_comments",
		mcp.WithDescription("List discussion comments (Legacy)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_list_discussion_comments_legacyHandler(cfg),
	}
}
