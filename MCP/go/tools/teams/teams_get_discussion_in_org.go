...
func Teams_get_discussion_in_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/teams/%s/discussions/%s", cfg.BaseURL, org, team_slug, discussion_number)
		...
		var result models.GeneratedType_Team_discussion
...
func CreateTeams_get_discussion_in_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_orgs_org_teams_team_slug_discussions_discussion_number_2",
		mcp.WithDescription("Get a discussion"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Teams_get_discussion_in_orgHandler(cfg),
	}
}
