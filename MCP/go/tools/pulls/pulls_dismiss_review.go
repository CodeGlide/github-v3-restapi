...
func Pulls_dismiss_reviewHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pulls/%s/reviews/%s/dismissals", cfg.BaseURL, owner, repo, pull_number, review_id)
		...
		var result models.GeneratedType_Pull_request_review
...
func CreatePulls_dismiss_reviewTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_repos_owner_repo_pulls_pull_number_reviews_review_id",
		mcp.WithDescription("Dismiss a review for a pull request"),
		...
		mcp.WithString("message", mcp.Required(), mcp.Description("Input parameter: The message for the pull request review dismissal")),
		mcp.WithString("event", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Pulls_dismiss_reviewHandler(cfg),
	}
}
