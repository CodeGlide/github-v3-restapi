...
func Pulls_remove_requested_reviewersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pulls/%s/requested_reviewers%s", cfg.BaseURL, owner, repo, pull_number, queryString)
...
func CreatePulls_remove_requested_reviewersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_pulls_pull_number_requested_reviewers",
		mcp.WithDescription("Remove requested reviewers from a pull request"),
		...
		mcp.WithArray("team_reviewers", mcp.Description("Input parameter: An array of team `slug`s that will be removed.")),
		mcp.WithArray("reviewers", mcp.Required(), mcp.Description("Input parameter: An array of user `login`s that will be removed.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Pulls_remove_requested_reviewersHandler(cfg),
	}
}
