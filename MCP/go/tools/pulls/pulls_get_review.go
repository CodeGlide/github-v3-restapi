...
func Pulls_get_reviewHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pulls/%s/reviews/%s", cfg.BaseURL, owner, repo, pull_number, review_id)
...
func CreatePulls_get_reviewTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_pulls_pull_number_reviews_review_id",
		mcp.WithDescription("Get a review for a pull request"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Pulls_get_reviewHandler(cfg),
	}
}
