...
func Pulls_request_reviewersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/pulls/%s/requested_reviewers", cfg.BaseURL, owner, repo, pull_number)
		...
		var result models.GeneratedType_Pull_request_simple
...
func CreatePulls_request_reviewersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_pulls_pull_number_requested_reviewers",
		mcp.WithDescription("Request reviewers for a pull request"),
		...
		mcp.WithArray("reviewers", mcp.Description("Input parameter: An array of user `login`s that will be requested.")),
		mcp.WithArray("team_reviewers", mcp.Description("Input parameter: An array of team `slug`s that will be requested.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Pulls_request_reviewersHandler(cfg),
	}
}
