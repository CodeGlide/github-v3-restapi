...
func Repos_create_commit_commentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		commit_shaVal, ok := args["commit_sha"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: commit_sha"), nil
		}
		commit_sha, ok := commit_shaVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: commit_sha"), nil
		}
		...
		url := fmt.Sprintf("%s/repos/%s/%s/commits/%s/comments%s", cfg.BaseURL, owner, repo, commit_sha, queryString)
...
func CreateRepos_create_commit_commentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_commits_commit_sha_comments",
		mcp.WithDescription("Create a commit comment"),
		...
		mcp.WithString("commit_sha", mcp.Required(), mcp.Description("The SHA of the commit.")),
		mcp.WithString("path", mcp.Description("Input parameter: Relative path of the file to comment on.")),
		mcp.WithNumber("position", mcp.Description("Input parameter: Line index in the diff to comment on.")),
		mcp.WithString("body", mcp.Required(), mcp.Description("Input parameter: The contents of the comment.")),
		mcp.WithNumber("line", mcp.Description("Input parameter: **Closing down notice**. Use **position** parameter instead. Line number in the file to comment on.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_create_commit_commentHandler(cfg),
	}
}
