...
func Repos_create_dispatch_eventHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/dispatches%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateRepos_create_dispatch_eventTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_dispatches",
		mcp.WithDescription("Create a repository dispatch event"),
		...
		mcp.WithString("event_type", mcp.Required(), mcp.Description("Input parameter: A custom webhook event name. Must be 100 characters or fewer.")),
		mcp.WithObject("client_payload", mcp.Description("Input parameter: JSON payload with extra information about the webhook event that your action or workflow may use. The maximum number of top-level properties is 10. The total size of the JSON payload must be less than 64KB.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_create_dispatch_eventHandler(cfg),
	}
}
