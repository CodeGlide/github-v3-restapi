...
func Repos_create_deploy_keyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/keys", cfg.BaseURL, owner, repo)
		...
		var result models.GeneratedType_Deploy_key
...
func CreateRepos_create_deploy_keyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_keys",
		mcp.WithDescription("Create a deploy key"),
		...
		mcp.WithString("title", mcp.Description("Input parameter: A name for the key.")),
		mcp.WithString("key", mcp.Required(), mcp.Description("Input parameter: The contents of the key.")),
		mcp.WithBoolean("read_only", mcp.Description("Input parameter: If `true`, the key will only be able to read repository contents. Otherwise, the key will be able to read and write.  \n  \nDeploy keys with write access can perform the same actions as an organization member with admin access, or a collaborator on a personal repository. For more information, see \"[Repository permission levels for an organization](https://docs.github.com/articles/repository-permission-levels-for-an-organization/)\" and \"[Permission levels for a user account repository](https://docs.github.com/articles/permission-levels-for-a-user-account-repository/).\"")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_create_deploy_keyHandler(cfg),
	}
}
