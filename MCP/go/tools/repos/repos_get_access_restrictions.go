...
func Repos_get_access_restrictionsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/branches/%s/protection/restrictions", cfg.BaseURL, owner, repo, branch)
		...
		var result models.GeneratedType_Branch_restriction_policy
...
func CreateRepos_get_access_restrictionsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_repos_owner_repo_branches_branch_protection_restrictions",
		mcp.WithDescription("Get access restrictions"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_get_access_restrictionsHandler(cfg),
	}
}
