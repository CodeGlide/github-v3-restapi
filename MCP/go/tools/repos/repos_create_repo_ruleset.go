...
func Repos_create_repo_rulesetHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/rulesets", cfg.BaseURL, owner, repo)
		...
		var result models.GeneratedType_Repository_ruleset
...
func CreateRepos_create_repo_rulesetTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_rulesets",
		mcp.WithDescription("Create a repository ruleset"),
		...
		mcp.WithObject("conditions", mcp.Description("Input parameter: Parameters for a repository ruleset ref name condition")),
		mcp.WithString("enforcement", mcp.Required(), mcp.Description("Input parameter: The enforcement level of the ruleset. `evaluate` allows admins to test rules before enforcing them. Admins can view insights on the Rule Insights page (`evaluate` is only available with GitHub Enterprise).")),
		mcp.WithString("name", mcp.Required(), mcp.Description("Input parameter: The name of the ruleset.")),
		mcp.WithArray("rules", mcp.Description("Input parameter: An array of rules within the ruleset.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_create_repo_rulesetHandler(cfg),
	}
}
