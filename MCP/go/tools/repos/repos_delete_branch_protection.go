...
func Repos_delete_branch_protectionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/branches/%s/protection", cfg.BaseURL, owner, repo, branch)
...
func CreateRepos_delete_branch_protectionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_repos_owner_repo_branches_branch_protection_2",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_delete_branch_protectionHandler(cfg),
	}
}
