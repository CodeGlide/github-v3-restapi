...
func Secret_scanning_create_push_protection_bypassHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/secret-scanning/push-protection-bypasses", cfg.BaseURL, owner, repo)
		...
		var result models.GeneratedType_Secret_scanning_push_protection_bypass
...
func CreateSecret_scanning_create_push_protection_bypassTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_secret-scanning",
		mcp.WithDescription("Create a push protection bypass"),
		...
		mcp.WithString("placeholder_id", mcp.Required(), mcp.Description("Input parameter: The ID of the push protection bypass placeholder. This value is returned on any push protected routes.")),
		mcp.WithString("reason", mcp.Required(), mcp.Description("Input parameter: The reason for bypassing push protection.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Secret_scanning_create_push_protection_bypassHandler(cfg),
	}
}
