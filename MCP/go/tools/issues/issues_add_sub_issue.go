...
func Issues_add_sub_issueHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/issues/%s/sub_issues%s", cfg.BaseURL, owner, repo, issue_number, queryString)
...
func CreateIssues_add_sub_issueTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_issues_issue_number_sub_issues",
		mcp.WithDescription("Add sub-issue"),
		...
		mcp.WithNumber("sub_issue_id", mcp.Required(), mcp.Description("Input parameter: The id of the sub-issue to add. The sub-issue must belong to the same repository owner as the parent issue")),
		mcp.WithBoolean("replace_parent", mcp.Description("Input parameter: Option that, when true, instructs the operation to replace the sub-issues current parent issue")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Issues_add_sub_issueHandler(cfg),
	}
}
