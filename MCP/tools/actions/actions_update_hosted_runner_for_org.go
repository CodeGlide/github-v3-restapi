...
func Actions_update_hosted_runner_for_orgHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/actions/hosted-runners/%s%s", cfg.BaseURL, org, hosted_runner_id, queryString)
...
func CreateActions_update_hosted_runner_for_orgTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_orgs_org_actions_hosted-runners_hosted_runner_id",
		mcp.WithDescription("Update a GitHub-hosted runner for an organization"),
		...
		mcp.WithString("name", mcp.Description("Input parameter: Name of the runner. Must be between 1 and 64 characters and may only contain upper and lowercase letters a-z, numbers 0-9, '.', '-', and '_'.")),
		mcp.WithNumber("runner_group_id", mcp.Description("Input parameter: The existing runner group to add this runner to.")),
		mcp.WithBoolean("enable_static_ip", mcp.Description("Input parameter: Whether this runner should be updated with a static public IP. Note limit on account. To list limits on account, use `GET actions/hosted-runners/limits`")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Actions_update_hosted_runner_for_orgHandler(cfg),
	}
}
