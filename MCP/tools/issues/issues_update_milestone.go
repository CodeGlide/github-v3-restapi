...
func Issues_update_milestoneHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/milestones/%s%s", cfg.BaseURL, owner, repo, milestone_number, queryString)
...
func CreateIssues_update_milestoneTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_repos_owner_repo_milestones_milestone_number",
		mcp.WithDescription("Update a milestone"),
		...
		mcp.WithString("title", mcp.Description("Input parameter: The title of the milestone.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Issues_update_milestoneHandler(cfg),
	}
}
