...
func Issues_create_labelHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/repos/%s/%s/labels%s", cfg.BaseURL, owner, repo, queryString)
...
func CreateIssues_create_labelTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_labels",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Issues_create_labelHandler(cfg),
	}
}
