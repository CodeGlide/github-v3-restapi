...
func Repos_create_pages_siteHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_create_pages_siteTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_repos_owner_repo_pages",
		mcp.WithDescription("Create a GitHub Pages site"),
		...
		mcp.WithObject("source", mcp.Description("Input parameter: The source branch and directory used to publish your Pages site.")),
		mcp.WithString("build_type", mcp.Description("Input parameter: The process in which the Page will be built. Possible values are `\"legacy\"` and `\"workflow\"`.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Repos_create_pages_siteHandler(cfg),
	}
}
