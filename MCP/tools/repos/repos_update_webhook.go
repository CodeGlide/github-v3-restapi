...
func Repos_update_webhookHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateRepos_update_webhookTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_repos_owner_repo_hooks_hook_id",
		mcp.WithDescription("Update a repository webhook"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Repos_update_webhookHandler(cfg),
	}
}
