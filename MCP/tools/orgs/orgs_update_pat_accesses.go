...
func Orgs_update_pat_accessesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/orgs/%s/personal-access-tokens%s", cfg.BaseURL, org, queryString)
...
func CreateOrgs_update_pat_accessesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_orgs_org_personal-access-tokens",
		mcp.WithDescription("Update the access to organization resources via fine-grained personal access tokens"),
		mcp.WithString("org", mcp.Required(), mcp.Description("The organization name. The name is not case sensitive.")),
		mcp.WithArray("pat_ids", mcp.Required(), mcp.Description("Input parameter: The IDs of the fine-grained personal access tokens.")),
		mcp.WithString("action", mcp.Required(), mcp.Description("Input parameter: Action to apply to the fine-grained personal access token.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Orgs_update_pat_accessesHandler(cfg),
	}
}
